...
def _vgg(arch, cfg, batch_norm, pretrained, device, **kwargs):
    ...
    if pretrained:
        checkpoint = os.path.join(ROOT_DIR, f"models/{arch}.pt")
...
def vgg13_bn(pretrained=False, device="cpu", **kwargs):
...
def vgg16_bn(pretrained=False, device="cpu", **kwargs):
...
def vgg19_bn(pretrained=False, device="cpu", **kwargs):
